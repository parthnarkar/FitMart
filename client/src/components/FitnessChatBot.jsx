...
export default function FitnessChatBot() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMsgs(prev => [
        ...prev,
        { role: "bot", text: "Sorry, I couldn't connect right now. Please try again.", error: true },
      ]);
    } finally {
      setTyping(false);
    }
  ...
  return (
    <>
      <style>{`
        ...
        .fm-chat-window.closed { opacity:0; transform:scale(0.92) translateY(12px); pointer-events:none; }
        .fm-chat-window.open   { opacity:1; transform:scale(1)    translateY(0);     pointer-events:auto; }
        .fm-fab {
          transition: transform 0.2s cubic-bezier(0.16,1,0.3,1), box-shadow 0.2s ease;
        }
        .fm-fab:hover { transform: scale(1.06); }
        .fm-fab:active { transform: scale(0.96); }
        .fm-msg { animation: fmMsgIn 0.3s cubic-bezier(0.16,1,0.3,1) forwards; }
        @keyframes fmMsgIn {
      ...
      {/* ── Chat window ──────────────────────────────────────────────────── */}
      <div
        className={`fm-chat-window fixed bottom-24 right-5 z-50 w-[360px] max-w-[calc(100vw-40px)]
                    bg-white border border-stone-200 rounded-2xl shadow-2xl flex flex-col
                    overflow-hidden ${open ? "open" : "closed"}`}
        style={{ fontFamily: "'DM Sans', sans-serif", height: "520px" }}
      >
        {/* Header */}
        <div className="bg-stone-900 px-5 py-4 flex items-center justify-between flex-shrink-0">
          <div>
            <p className="text-xs tracking-[0.2em] uppercase text-stone-400 mb-0.5">FitMart</p>
            <h3
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="text-stone-400 hover:text-white transition-colors text-xl leading-none"
            >
              ×
            </button>
          </div>
        ...
        {/* Messages */}
        <div className="flex-1 overflow-y-auto px-4 py-5 space-y-3 fm-scrollbar bg-stone-50">
          {msgs.map((msg, i) => (
            <div
              key={i}
            ...
            >
              ...
              {msg.role === "bot" && (
                <div className="w-7 h-7 rounded-full bg-stone-900 flex items-center justify-center
                                text-white text-xs flex-shrink-0 mr-2 mt-0.5">
                  ◎
                </div>
              )}
        ...
        {/* Input area */}
        <div className="px-4 py-3 bg-white flex items-end gap-2 flex-shrink-0">
          <textarea
            ref={inputRef}
            rows={1}
            value={input}
            ...
            className="flex-1 resize-none border border-stone-200 bg-stone-50 rounded-xl
                       px-4 py-2.5 text-sm text-stone-900 placeholder-stone-300
          ...
          <button
            ...
            className="w-9 h-9 bg-stone-900 text-white rounded-full flex items-center
                       justify-center hover:bg-stone-700 transition-colors
