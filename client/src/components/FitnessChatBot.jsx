...
export default function FitnessChatBot() {
  ...
  // **Format message text with bold support**
    ...
    return lines.map((line, lineIndex) => {
      ...
      while ((matchFound = boldRegex.exec(line)) !== null) {
        matches.push({
          ...
          fullMatch: matchFound[0]
        });
      }

      if (matches.length === 0) {
        // No bold text, return plain line
        parts.push(<span key={`line-${lineIndex}`}>{line}</span>);
      } else {
        // Process with bold segments
        let currentPos = 0;
        matches.forEach((match, idx) => {
          ...
          // Add bold text
          parts.push(
            <strong key={`bold-${lineIndex}-${idx}`} className="font-semibold text-stone-900">
              {match.text}
            </strong>
          );
  ...
  return (
    <>
      ...
      {/* **Chat Window** */}
      <div
        className={`fm-chat-window fixed bottom-24 right-5 z-50 w-90 max-w-[calc(100vw-40px)]
      ...
      >
            ...
            >
              ...
              {msg.role === "bot" && (
                <div className="w-7 h-7 rounded-full bg-stone-900 flex items-center justify-center
                                text-white text-xs shrink-0 mr-2 mt-0.5">
                  ◎
                </div>
              )}
        ...
        {/* **Input Area** */}
        <div className="px-4 py-3 bg-white flex items-end gap-2 shrink-0">
          <textarea
            ref={inputRef}
            rows={1}
            value={input}
