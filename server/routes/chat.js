  ...
  try {
    ...
    if (wantsProduct) {
      try {
        ...
        if (product) {
          reply += `\n\n💪 **Recommended Product:** ${product.name}${product.brand ? ` by ${product.brand}` : ""}${product.price ? ` — ₹${product.price.toLocaleString("en-IN")}` : ""}`;
          if (product.rating) {
            reply += ` ⭐ ${product.rating}/5`;
          }
        }
      } catch (productError) {
    ...
    if (usedFallback) {
      reply += "\n\n_Note: Using enhanced knowledge base. For more detailed responses, ensure API key has available quota._";
    }
